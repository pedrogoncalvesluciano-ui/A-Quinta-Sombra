...
// =========================================================
// 0.6.51 — PARTE 4/5
// CAP. 7 "RETORNO" + CAP. 8 "O QUE FICA PERTO DE CASA"
// =========================================================

let v0651HouseStaticUntil = 0;
let v0651AtticStaticUntil = 0;

function v0651Chapter7Unlocked() {
  return Boolean(
    state &&
    state.stage !== "prologue" &&
    state.day >= 14 &&
...
function v0651Chapter8Unlocked() {
  return Boolean(
    state &&
    state.stage !== "prologue" &&
    state.day >= 17 &&
...
const v0651PrepareBase = prepareSystems;
prepareSystems = function() {
  v0651PrepareBase();
  ...
  if (!state.chapter7 || typeof state.chapter7 !== "object") {
    state.chapter7 = {
      ...
      complete: false,
      fatherRoom: "foyer",
      fatherX: housePoint(330),
      fatherY: housePoint(230),
      followDelay: 0,
      lastPlayerRoom: null,
      caught: false
    };
  ...
  for (const key of [
    ...
    if (typeof c7[key] !== "boolean") {
      c7[key] = false;
    }
  }

  if (typeof c7.phase !== "string") {
    c7.phase = "waiting";
  }

  if (!Number.isFinite(c7.fatherX)) {
    c7.fatherX = housePoint(330);
  }

  if (!Number.isFinite(c7.fatherY)) {
    c7.fatherY = housePoint(230);
  }

  if (!Number.isFinite(c7.followDelay)) {
    c7.followDelay = 0;
  }

  if (v0651Chapter7Unlocked() && c7.phase === "waiting") {
    c7.phase = "knocking";
    c7.fatherRoom = "foyer";
    c7.fatherX = housePoint(330);
    c7.fatherY = housePoint(230);
    c7.lastPlayerRoom = state.room;
  }

  if (!state.chapter8 || typeof state.chapter8 !== "object") {
    state.chapter8 = {
      atticShaftSeen: false,
      florindaConfronted: false,
      neighborKey: false,
      splitRevealed: false,
      motherTrailSeen: false,
      complete: false
    };
  }
...
function v0651FatherAtFoyer() {
  return Boolean(
    state?.chapter7 &&
    ["inside", "photoCheck"].includes(state.chapter7.phase) &&
    state.room === "foyer"
  );
}

function v0651OpenFatherDoor() {
  prepareSystems();

  if (state.chapter7.phase !== "knocking") return;

  say(
    [
      ["Você", "Pai...?"],
      ["Pai", "Estevão."],
      ["Você", "Onde você estava? Cadê a mãe?"],
      ["Pai", "Eu não sei. Eu acordei longe daqui e só consegui voltar agora."],
      ["Você", "Você sumiu por quase duas semanas."],
      ["Pai", "Eu sei. Depois eu explico. Primeiro eu preciso entrar."]
    ],
    () => {
      state.chapter7.phase = "inside";
      state.chapter7.fatherRoom = "foyer";
      state.chapter7.fatherX = housePoint(335);
      state.chapter7.fatherY = housePoint(225);
      state.chapter7.lastPlayerRoom = state.room;
...
function v0651BrotherWarnsCopy() {
  prepareSystems();

  if (state.chapter7.brotherWarned) {
    say([
      ["Irmão", "Compara com a foto."],
      ["Irmão", "Eu sei que parece ele. Mas tem alguma coisa errada."]
    ]);
    return;
  }
  ...
  say(
    [
      ...
      ["Irmão", "Não."],
      ["Você", "Como assim?"],
      ["Irmão", "Ele entrou e nem perguntou onde eu estava."],
      ["Irmão", "Ele sempre pergunta primeiro."],
      ["Você", "Ele está confuso. Ficou desaparecido esse tempo todo."],
      ["Irmão", "Compara com a foto. O bilhete disse pra fazer isso, lembra?"]
    ],
    () => {
      state.chapter7.brotherWarned = true;
      state.chapter7.phase = "photoCheck";
      updateHud();
      save();
    }
  );
...
function v0651StartDomesticChase() {
  const c7 = state.chapter7;

  c7.phase = "chase";
  c7.hideUnlocked = true;
  c7.brotherSafe = false;
  c7.caught = false;
  c7.fatherRoom = state.room;
  c7.fatherX = Math.max(
    housePoint(80),
    state.x - housePoint(105)
  );
  c7.fatherY = state.y;
  c7.followDelay = 0;
  c7.lastPlayerRoom = state.room;

  v0651HouseStaticUntil = elapsed + 1.1;
  keys.clear();

  v06Toast(
    "Leve seu irmão para um lugar seguro.",
    2.8
  );
...
function v0651CompareFatherPhoto() {
  prepareSystems();

  if (state.chapter7.phase !== "photoCheck") return;

  const q = chapter();

  if (!q?.clues?.includes("photo")) {
    say([
      "O bilhete fala de uma fotografia. Preciso encontrar a que estava no quarto dos meus pais."
    ]);
    return;
  }
  ...
  say(
    [
      ["Você", "Pai. Você lembra dessa foto? Do meu aniversário?"],
      ["Pai", "Claro. Foi naquela tarde na praça."],
      ["Você", "Não foi na praça."],
      ["Pai", "...Foi perto da escada."],
      ["Você", "Você só disse isso depois que eu corrigi."],
      ["Pai", "Estevão, chega."],
    ...
    () => {
      state.chapter7.photoUsed = true;
      v0650EnsureInvestigationLog().keyClues.photoCopy = true;
      v0651StartDomesticChase();
    }
  );
}

function v0651ProtectBrother() {
  prepareSystems();

  if (
    state.chapter7.phase !== "chase" ||
    state.chapter7.brotherSafe
  ) {
    return;
  }
  ...
  say(
    [
      ...
      ["Você", "Não olha pra ele. Vai pro sótão e fica atrás dos móveis cobertos."],
      ["Irmão", "E você?"],
      ["Você", "Eu vou logo atrás."]
    ],
    () => {
      state.chapter7.brotherSafe = true;
      v06Toast("Seu irmão chegou ao sótão.", 2);
      updateHud();
      save();
    }
  );
...
function v0651FinishDomesticChase() {
  prepareSystems();
  ...
  keys.clear();
  v0651HouseStaticUntil = elapsed + 1.2;

  say(
    [
      "Prendo a respiração atrás dos móveis cobertos.",
      "Passos param do outro lado da porta.",
      "A estática cresce por alguns segundos.",
      "Então tudo fica em silêncio."
    ],
    () => {
      state.chapter7.phase = "escaped";
      state.chapter7.complete = true;
      state.chapter7.fatherRoom = "";
      state.chapter7.caught = false;

      v06Toast("Capítulo 7 concluído · Retorno", 2.5);
      updateHud();
      save();
    }
  );
...
function v0651FatherCaught() {
  ...
  modal(
    "Ele te alcançou",
    "A mão dele fecha no seu braço antes de você conseguir colocar seu irmão em segurança.",
    [
      [
        "Tentar novamente",
        () => {
          ...
          c7.caught = false;
          c7.fatherRoom = "foyer";
          c7.fatherX = housePoint(315);
          c7.fatherY = housePoint(220);
          c7.followDelay = 0;
          c7.lastPlayerRoom = "foyer";

          state.room = "foyer";
          state.x = housePoint(430);
          state.y = housePoint(270);
          state.facing = "left";
          state.walk = 0;
...
function v0651FatherSpawnForRoom(roomId) {
  const m = maps[roomId];
  if (!m) return null;

  const positions = {
    foyer: [housePoint(345), housePoint(260)],
    hall: [housePoint(390), housePoint(250)],
    brother: [housePoint(470), housePoint(300)],
    bedroom: [housePoint(470), housePoint(290)],
    parents: [housePoint(450), housePoint(300)],
    living: [housePoint(500), housePoint(220)],
    kitchen: [housePoint(310), housePoint(300)],
    attic: [housePoint(310), housePoint(320)]
  };

  const p = positions[roomId] || [
    Math.max(housePoint(70), Math.min(m.w - housePoint(70), state.x - housePoint(110))),
    Math.max(housePoint(70), Math.min(m.h - housePoint(70), state.y))
  ];

  return { x: p[0], y: p[1] };
}

function v0651MoveFather(dt) {
  const c7 = state.chapter7;
  ...
  if (c7.lastPlayerRoom !== state.room) {
    c7.lastPlayerRoom = state.room;
    c7.followDelay = state.room === "attic" ? 1.35 : 0.85;
  }

  if (c7.fatherRoom !== state.room) {
    c7.followDelay = Math.max(
      0,
      c7.followDelay - dt
    );
    ...
    const spawn = v0651FatherSpawnForRoom(state.room);
    if (!spawn) return;
  ...
  const dx = state.x - c7.fatherX;
  const dy = state.y - c7.fatherY;
  const distance = Math.hypot(dx, dy) || 1;
  ...
  // O perseguidor respeita as colisões do mesmo cômodo.
  if (!solid(c7.fatherX + stepX, c7.fatherY)) {
    c7.fatherX += stepX;
  }

  if (!solid(c7.fatherX, c7.fatherY + stepY)) {
    c7.fatherY += stepY;
  }
}

function v0651InspectAtticShaft() {
  prepareSystems();

  if (state.chapter8.atticShaftSeen) {
    say([
      "A estrutura dentro da parede desce na mesma direção do porão.",
      "A casa parece ter sido construída ao redor de algo mais antigo."
    ]);
    return;
  }
  ...
  say(
    [
      "Atrás de um dos móveis cobertos, a parede não é maciça.",
      "Há vigas verticais antigas e um espaço estreito descendo por dentro da casa.",
      "O alinhamento é o mesmo da parede onde fica o armário do porão.",
      "Isso já existia antes de a minha família reformar a casa."
    ],
    () => {
      state.chapter8.atticShaftSeen = true;
      v0651AtticStaticUntil = elapsed + 0.7;
      updateHud();
      save();
    }
  );
...
function v0651FlorindaConfession() {
  prepareSystems();

  if (state.chapter8.florindaConfronted) {
    say([
      ["Florinda", "Eu contei o que prometi à sua mãe."],
      ["Florinda", "Se você apagar perto do amanhecer, eu vou buscar você. Sempre foi isso."]
    ]);
    return;
  }
  ...
  say(
    [
      ["Você", "Você sabia. Sobre as sete horas. Sabia o tempo todo."],
      ["Florinda", "Eu prometi aos seus pais que não deixaria você lá fora quando isso acontecesse."],
      ["Você", "Isso não explica por que eu sempre acordo em casa sem lembrar de nada."],
      ["Florinda", "Porque eu vou até você."],
      ["Florinda", "Sua mãe me deu uma cópia da chave. Eu te trago pra dentro antes que alguém mais... note."],
      ["Você", "Note o quê?"],
      ["Florinda", "Eu não sei o nome disso."],
      ["Florinda", "Só sei que seus pais estavam com medo de alguma coisa. E que sua mãe me fez prometer que eu cuidaria de vocês."],
      ["Você", "Por que não me contou antes?"],
      ["Florinda", "Porque eles me pediram para não colocar isso na cabeça de vocês sem prova."]
    ],
    () => {
      state.chapter8.florindaConfronted = true;
      state.chapter8.neighborKey = true;
      v0650EnsureInvestigationLog().keyClues.florindaConfession = true;

      v06Toast("Florinda revelou a verdade sobre os retornos às 07h.", 2.6);
      updateHud();
      save();
    }
  );
...
function v0651RaimundoSplitReveal() {
  prepareSystems();

  if (state.chapter8.splitRevealed) {
    say([
      ["Raimundo", "Split Lancaster. Esse nome eu não esqueço."],
      ["Raimundo", "Quarenta anos não foram suficientes pra esquecer."]
    ]);
    return;
  }
  ...
  say(
    [
      ["Raimundo", "Seu nome... seu sobrenome é familiar demais."],
      ["Você", "Você já falou isso antes."],
      ["Raimundo", "Há quarenta anos eu trabalhava naquela mina."],
      ["Raimundo", "O capataz se chamava Split Lancaster."],
      ["Você", "Lancaster?"],
      ["Raimundo", "Duro. Não deixava passar erro nenhum. Nem quando os homens diziam que a galeria estava rachando."],
      ["Você", "Ele era parente meu?"],
      ["Raimundo", "Se essa casa veio pra sua família pelo seu pai... era o pai dele."],
      ["Você", "Meu avô."],
      ["Raimundo", "Eu já parei de acreditar em coincidência faz uns quarenta anos."]
    ],
    () => {
      state.chapter8.splitRevealed = true;
      v0650EnsureInvestigationLog().keyClues.splitReveal = true;
      updateHud();
      save();
    }
...
function v0651MotherTrailAtCabinet() {
  prepareSystems();

  if (state.chapter8.motherTrailSeen) {
    say([
      "O pedaço de tecido continua preso atrás do armário.",
      "É do casaco da minha mãe. Ela passou por aqui."
    ]);
    return;
  }
  ...
  say(
    [
      "Agora que sei que minha mãe tinha uma chave com Florinda, os riscos no chão parecem ainda menos acidentais.",
      "Entre a lateral do armário e a parede há um pedaço de tecido preso.",
      "Eu reconheço a costura.",
      "É do casaco da minha mãe.",
      "Ela esteve atrás desse armário.",
      "Se existe uma passagem ali, talvez ela ainda esteja do outro lado."
    ],
    () => {
      state.chapter8.motherTrailSeen = true;
      state.chapter8.complete = true;
      v0651HouseStaticUntil = elapsed + 0.85;

      v06Toast("Capítulo 8 concluído · algo existe atrás do armário.", 2.8);
      updateHud();
      save();
    }
  );
...
const v0651GetNearBase = getNear;
getNear = function() {
  prepareSystems();

  const baseTarget = v0651GetNearBase();

  if (
    state?.room === "foyer" &&
    state.chapter7?.phase === "knocking" &&
    baseTarget?.action === "outside"
  ) {
    return {
      ...baseTarget,
      label: "Abrir a porta",
      action: "fatherReturnDoor"
    };
  }
  ...
  if (
    v0651FatherAtFoyer() &&
    Math.hypot(
      state.x - state.chapter7.fatherX,
      state.y - state.chapter7.fatherY
    ) < 46
  ) {
    return {
      label: "Falar com seu pai",
      action: "fatherCopy"
    };
  }
  ...
  if (
    ...
    const bed = maps.brother.objects.find(
      o => o.type === "brotherbed"
    );
    ...
    if (bed) {
      const px = Math.max(
        bed.x,
        Math.min(state.x, bed.x + bed.w)
      );
      const py = Math.max(
        bed.y,
        Math.min(state.y, bed.y + bed.h)
      );

      if (Math.hypot(state.x - px, state.y - py) < 48) {
        return {
          label: "Proteger seu irmão",
          action: "protectBrother"
        };
      }
    }
  }
  ...
  if (
    ...
    state.room === "attic" &&
    Math.hypot(
      state.x - housePoint(285),
      state.y - housePoint(245)
    ) < 58
  ) {
    return {
      label: "Esconder-se atrás dos móveis",
      action: "hideAttic"
    };
  }

  if (
    v0651Chapter8Unlocked() &&
    state.room === "attic" &&
    Math.hypot(
      state.x - housePoint(500),
      state.y - housePoint(255)
    ) < 54
  ) {
    return {
      label: "Examinar a parede atrás dos móveis",
...
const v0651InteractBase = interact;
interact = function(action) {
  prepareSystems();

  if (action === "fatherReturnDoor") {
    v0651OpenFatherDoor();
    return;
  }
  ...
  if (
    action === "outside" &&
    state.chapter7 &&
    ["inside", "photoCheck", "chase"].includes(state.chapter7.phase)
  ) {
    say([
      state.chapter7.phase === "chase"
        ? "Não vou deixar meu irmão aqui com ele."
        : "Antes de sair, preciso entender o que está acontecendo dentro de casa."
    ]);
    return;
  }
  ...
  if (
    ...
    v0651BrotherWarnsCopy();
    return;
  }

  if (action === "fatherCopy") {
    if (state.chapter7.phase === "inside") {
      say([
        ["Pai", "Depois a gente conversa. Eu só preciso descansar um pouco."],
        ["Você", "Cadê a mãe?"],
        ["Pai", "Eu já disse que não sei."]
      ]);
      return;
    }

    if (state.chapter7.phase === "photoCheck") {
      v0651CompareFatherPhoto();
      return;
    }
  }

  if (action === "protectBrother") {
    v0651ProtectBrother();
    return;
  }

  if (action === "hideAttic") {
    v0651FinishDomesticChase();
    return;
  }

  if (action === "atticShaft") {
    v0651InspectAtticShaft();
    return;
  }
  ...
  if (
    action === "vendor" &&
    v0651Chapter8Unlocked() &&
    state.chapter8.atticShaftSeen &&
    !state.chapter8.florindaConfronted
  ) {
    v0651FlorindaConfession();
    return;
  }
  ...
  if (
    ...
    v0651Chapter8Unlocked() &&
    state.chapter6?.notebookRead &&
    !state.chapter8.splitRevealed
  ) {
    v0651RaimundoSplitReveal();
    return;
  }
  ...
  if (
    ...
    v0651MotherTrailAtCabinet();
    return;
  }

  v0651InteractBase(action);
};

function v0651DrawFatherCopy() {
  if (!state?.chapter7) return;

  const c7 = state.chapter7;

  const visible =
    (
      ["inside", "photoCheck"].includes(c7.phase) &&
      state.room === "foyer"
    ) ||
    (
      c7.phase === "chase" &&
  ...
  person(
    ...
    c7.fatherY,
    "father",
    c7.phase === "chase" ? elapsed * 10 : 0,
    c7.phase === "chase"
      ? (
          Math.abs(state.x - c7.fatherX) >
          Math.abs(state.y - c7.fatherY)
            ? (state.x > c7.fatherX ? "right" : "left")
            : (state.y > c7.fatherY ? "down" : "up")
        )
      : "down",
    0.98
  );
...
function v0651DrawAtticConnection() {
  if (
    !state ||
    state.room !== "attic" ||
    !v0651Chapter8Unlocked()
  ...
  const x = housePoint(515);
  const y = housePoint(205);

  rect(x, y, housePoint(7), housePoint(118), "#2d302e");
  rect(x + housePoint(13), y, housePoint(5), housePoint(118), "#373935");
...
function v0651DrawHouseStatic() {
  const until = Math.max(
    v0651HouseStaticUntil,
    v0651AtticStaticUntil
  );
  ...
  const strength = Math.min(
    1,
    Math.max(until - elapsed, 0) / 1.2
  );

  for (let i = 0; i < 22; i++) {
    const y =
      (i * 21 + Math.floor(elapsed * 760) % H) % H;
...
const v0651DrawBase = drawWorld;
drawWorld = function() {
  v0651DrawBase();

  v0651DrawFatherCopy();
  v0651DrawAtticConnection();
  v0651DrawHouseStatic();
};

const v0651UpdateBase = update;
update = function(dt) {
  prepareSystems();
  v0651UpdateBase(dt);
...
updateHud = function() {
  ...
  const c7 = state.chapter7;
  const c8 = state.chapter8;

  if (
    v0651Chapter7Unlocked() &&
    !c7.complete
  ) {
    if (c7.phase === "knocking") {
      $("objective").textContent =
        "Há alguém batendo na porta de casa.";
      return;
    }
    ...
    if (
      ...
      $("objective").textContent =
        "Converse com seu irmão sobre a volta do seu pai.";
      return;
    }

    if (c7.phase === "photoCheck") {
      $("objective").textContent =
        "Compare a fotografia do aniversário com seu pai.";
      return;
    }
    ...
    if (
      ...
      $("objective").textContent =
        "Encontre seu irmão e coloque-o em segurança.";
      return;
    }
    ...
    if (
      ...
      $("objective").textContent =
        "Suba ao sótão e esconda-se atrás dos móveis cobertos.";
      return;
    }
  }
  ...
  if (
    ...
    if (!c8.atticShaftSeen) {
      $("objective").textContent =
        "Volte ao sótão e examine a parede atrás dos móveis.";
      return;
    }

    if (!c8.florindaConfronted) {
      $("objective").textContent =
        "Confronte Florinda com o que você descobriu.";
      return;
    }

    if (!c8.motherTrailSeen) {
      $("objective").textContent =
        "Volte ao porão e examine novamente o armário.";
      return;
    }
  }
  ...
  if (c8.complete) {
    $("objective").textContent =
      "Há uma passagem atrás do armário. Preciso descobrir como abri-la.";
  }
};

// Inventário: mostra somente itens físicos; as seis pistas-chave continuam
// sendo variáveis internas, nunca um placar visível.
const v0651InventoryBase = v0645OpenInventory;
v0645OpenInventory = function() {
  ...
  if (state.chapter8?.neighborKey) {
    const root = $("modalText");
    const keyLine = document.createElement("div");
    keyLine.style.marginTop = "10px";
    keyLine.textContent =
      "• Cópia da chave da casa — entregue por Florinda";
    root.append(keyLine);
  }
};

// I chama v0645OpenInventory dinamicamente; J continua usando o diário
// implementado na Parte 3.
openJournal = function() {
  v0650OpenJournal();
};

// Comandos de QA das Partes 4.
const v0651DevBase = v0645RunDevCommand;
v0645RunDevCommand = function(raw) {
  const command = String(raw || "").trim().toLowerCase();
  ...
  if (command === "retorno") {
    ...
    state.chapter7.phase = "knocking";
    state.chapter7.brotherWarned = false;
    state.chapter7.photoUsed = false;
    state.chapter7.brotherSafe = false;
    state.chapter7.hideUnlocked = false;
    state.chapter7.complete = false;
    state.chapter7.caught = false;

    state.room = "foyer";
    state.x = housePoint(520);
    state.y = housePoint(300);
    state.facing = "right";
    state.walk = 0;
    ...
    updateHud();
    save();
    v06Toast("TESTE: Capítulo 7 · Retorno", 2);
    return;
  }
  ...
  if (command === "perto") {
    ...
    state.chapter7.photoUsed = true;
    state.chapter7.brotherSafe = true;
    state.investigationLog.keyClues.photoCopy = true;

    state.chapter8.atticShaftSeen = false;
    state.chapter8.florindaConfronted = false;
    state.chapter8.neighborKey = false;
    state.chapter8.splitRevealed = false;
    state.chapter8.motherTrailSeen = false;
    state.chapter8.complete = false;

    state.room = "attic";
    state.x = housePoint(420);
    state.y = housePoint(250);
    state.facing = "right";
    state.walk = 0;
    ...
    updateHud();
    save();
    v06Toast("TESTE: Capítulo 8 · Perto de Casa", 2);
    return;
  }
...
const v0651DevPanelBase = v0645OpenDevPanel;
v0645OpenDevPanel = function() {
  v0651DevPanelBase();

  const pre = $("modalText").querySelector("pre");

  if (pre && !pre.textContent.includes("retorno")) {
    pre.textContent +=
      "\nretorno     → testa o Capítulo 7 · cópia do pai" +
      "\nperto       → testa o Capítulo 8 · sótão/Florinda";
  }
};

// Ajuda final da Parte 4.
$("help").onclick = () => modal(
  "Como jogar",
  "WASD / setas: andar. Shift/F: correr quando disponível. E: interagir. I: inventário. J: diário. L: ligar/desligar a lanterna. Esc: pausar.\n\nA segunda metade do jogo passa a recompensar comparação de relatos, registros físicos e pistas já encontradas. Durante perseguições dentro de casa, proteja seu irmão antes de procurar um esconderijo.\n\nÀs 07:00, Estevão continua perdendo os sentidos.",
  [["Voltar", closeModal]]
);
